...
import { 
  ...
  Square, RefreshCw, Scissors, Type, Palette
} from 'lucide-react';
...
export const DocumentEditor: React.FC<DocumentEditorProps> = ({ htmlContent, onContentChange, editorRef }) => {
  ...
  // Setup image handlers and make images non-editable
  useEffect(() => {
    pageRefs.current.forEach(pageRef => {
      if (pageRef) {
        // Make all images non-editable so they don't interfere with contentEditable
        const images = pageRef.querySelectorAll('img');
        images.forEach(img => {
          const htmlImg = img as HTMLImageElement;
          htmlImg.contentEditable = 'false';
  ...
  const handleInput = (pageIndex: number) => {
    ...
    debounceRef.current = setTimeout(async () => {
        // Collect full HTML
        // Filter out null refs in case page count reduced
                        ...
                        <div 
                            ...
                            onClick={(e) => {
                              ...
                              if (target.tagName === 'IMG') {
                                ...
                                // Make image non-editable and add resize wrapper
