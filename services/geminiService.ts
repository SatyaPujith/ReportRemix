...
export const getDocumentEditSuggestions = async (
  ...
  3. **Style Preservation**: If an element has a 'style' attribute (e.g., style="text-align: center"), YOU MUST PRESERVE IT in the output unless explicitly asked to change the alignment.
  4. **Clean Output**: Return *ONLY* the HTML content. Do not wrap in \`\`\`html code blocks. Do not add markdown. Just the raw HTML.
  5. **IMAGE SAFETY**: You will see image tags like <img src="__IMG_PLACEHOLDER_0__">. **DO NOT CHANGE, REMOVE, OR REORDER THESE SRC ATTRIBUTES**. You must output the img tag exactly as is with the placeholder src.
  6. **NO TRUNCATION**: If the document has multiple pages or sections, return ALL of them. Do not summarize or shorten the document unless explicitly asked.
