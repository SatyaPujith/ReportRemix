...
export const paginateHtml = async (fullHtml: string): Promise<string[]> => {
    ...
    if (hasExplicitBreaks) {
        ...
        Array.from(container.childNodes).forEach(node => {
            if (node.nodeType === Node.ELEMENT_NODE) {
                ...
                if (element.classList.contains('word-page-break') || 
                    element.classList.contains('page-break-marker')) {
                    
                    // Save current page
                    if (currentPageContent.length > 0) {
                        const pageDiv = document.createElement('div');
                        currentPageContent.forEach(el => pageDiv.appendChild(el.cloneNode(true)));
    ...
    // Minus top/bottom margins (25.4mm each): 297 - 50.8 = 246.2mm
    // Convert to pixels at 96 DPI: 246.2mm * 3.7795 ≈ 930px
...
export const generatePdf = async (container: HTMLElement | null, filename: string): Promise<void> => {
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    try {
      ...
      const yOffset = imgHeight < a4Height ? (a4Height - imgHeight) / 2 : 0;
